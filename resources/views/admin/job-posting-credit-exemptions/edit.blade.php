<x-layouts.app>
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">{{ __('Guthabenausnahme bearbeiten') }}</h1>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 max-w-2xl">
        <form method="POST" action="{{ route('admin.job-posting-credit-exemptions.update', $jobPostingCreditExemption) }}">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select id="employment_type" name="employment_type" required
                        ...
                        @foreach($employmentTypes as $value => $label)
                            <option value="{{ $value }}" {{ old('employment_type', $jobPostingCreditExemption->employment_type) == $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="applies_to" name="applies_to" required
                        ...
                        @foreach($appliesTo as $value => $label)
                            <option value="{{ $value }}" {{ old('applies_to', $jobPostingCreditExemption->applies_to) == $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex items-start">
                    <div class="flex items-center h-5">
                        <input id="is_active" name="is_active" type="checkbox" value="1"
                               {{ old('is_active', $jobPostingCreditExemption->is_active) ? 'checked' : '' }}
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                              placeholder="{{ __('Optionale Beschreibung für interne Notizen...') }}">{{ old('description', $jobPostingCreditExemption->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div class="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 space-y-2">
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        <span class="font-medium">{{ __('Erstellt am:') }}</span>
                        {{ $jobPostingCreditExemption->created_at->format('d.m.Y H:i') }}
                    </p>
                    @if($jobPostingCreditExemption->creator)
                        <p class="text-sm text-gray-600 dark:text-gray-400">
                            <span class="font-medium">{{ __('Erstellt von:') }}</span>
                            {{ $jobPostingCreditExemption->creator->name }}
                        </p>
                    @endif
                </div>
