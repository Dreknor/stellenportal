...
<head>
    ...
    <title>Stellenausschreibung läuft bald ab</title>
</head>
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    <div style="background-color: #f4f4f4; padding: 20px; border-radius: 5px;">
        <h2 style="color: #2c3e50; margin-top: 0;">Stellenausschreibung läuft bald ab</h2>

        <p>Hallo {{ $user->first_name }} {{ $user->last_name }},</p>

        <p>Ihre Stellenausschreibung läuft in <strong>{{ $daysUntilExpiration }} {{ $daysUntilExpiration == 1 ? 'Tag' : 'Tagen' }}</strong> ab.</p>
        ...
        <div style="background-color: #fff; padding: 15px; border-left: 4px solid #f39c12; margin: 20px 0;">
            ...
            <p style="margin: 5px 0;"><strong>Läuft ab am:</strong> {{ $jobPosting->expires_at?->format('d.m.Y') }}</p>
        </div>

        <p>Möchten Sie diese Stellenausschreibung verlängern? Sie können die Laufzeit jederzeit mit zusätzlichen Credits erweitern.</p>

        <p style="margin-top: 30px;">
            <a href="{{ route('job-postings.show', $jobPosting) }}" style="display: inline-block; padding: 12px 24px; background-color: #3490dc; color: #fff; text-decoration: none; border-radius: 5px;">Stellenausschreibung verlängern</a>
        </p>
